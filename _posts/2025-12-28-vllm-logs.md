...
- **FLASH_ATTN**: [FlashAttention](https://github.com/Dao-AILab/flash-attention) CUDA implementation. Automatically uses FlashAttention-2 or FlashAttention-3 based on GPU architecture. Set `VLLM_FLASH_ATTN_VERSION=3` to force FA3 on H100/H800.
- **FLASHINFER**: [FlashInfer](https://github.com/flashinfer-ai/flashinfer) backend with different optimization strategies.

FlashAttention-3 ([paper](https://arxiv.org/abs/2407.08608), [vLLM integration](https://github.com/vllm-project/vllm/issues/12429)) is optimized for Hopper GPUs (H100) using WGMMA async Tensor Cores and TMA. Achieves [75-85% of H100 theoretical max FLOPS](https://tridao.me/blog/2024/flash3/) (740-850 TFLOPs) vs. 35% for FlashAttention-2. Enable with `VLLM_ATTENTION_BACKEND=FLASH_ATTN` + `VLLM_FLASH_ATTN_VERSION=3` (requires CUDA ≥12.3).
