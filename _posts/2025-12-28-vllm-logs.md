...
`max_num_batched_tokens` caps tokens per scheduler step. With [chunked prefill](https://docs.vllm.ai/en/stable/configuration/optimization/#chunked-prefill) enabled (default in V1), the scheduler prioritizes decode requests and batches pending prefills into remaining token budget. Lower values (e.g., 2048) achieve better ITL (inter-token latency) because there are fewer prefills interrupting decodes. Higher values achieve better TTFT (time-to-first-token) as more prefill tokens are processed per batch. Default is 8192 for online serving, 16384 for offline. See [vLLM Optimization docs](https://docs.vllm.ai/en/stable/configuration/optimization/#performance-tuning-with-chunked-prefill).

This value also affects how much memory is left for KV cache. vLLM [allocates all remaining GPU memory to KV cache](https://github.com/vllm-project/vllm/issues/20256) after loading model weights—controlled by `gpu_memory_utilization` (default 0.9). Higher `max_num_batched_tokens` reserves more activation memory during the profiling step, leaving less for KV cache and reducing `max_concurrency`. Decreasing `max_num_batched_tokens` or `max_num_seqs` [frees KV cache space](https://docs.vllm.ai/en/stable/configuration/optimization/#preemption) for more concurrent requests.
...
`max_num_seqs` caps concurrent sequences per batch (default 1024 in V1, up from 256 in V0). Higher values allow more concurrent requests but [require more KV cache space](https://docs.vllm.ai/en/stable/configuration/optimization/#preemption) at runtime. Lower values [reduce memory pressure](https://docs.vllm.ai/en/stable/configuration/optimization/#preemption) and avoid [preemption](https://docs.vllm.ai/en/stable/configuration/optimization/#preemption), where requests are evicted and recomputed when KV cache fills. Here, `max_num_seqs=6` is set low for this memory-constrained single-GPU setup.
...
- **FLASH_ATTN**: [FlashAttention](https://github.com/Dao-AILab/flash-attention) CUDA implementation. [H100/H800 GPUs default to FlashAttention-3 automatically](https://github.com/vllm-project/vllm/issues/22610)—no need to set `VLLM_FLASH_ATTN_VERSION=3`. FlashAttention-3 ([paper](https://arxiv.org/abs/2407.08608), [vLLM integration](https://github.com/vllm-project/vllm/issues/12429)) is optimized for Hopper using WGMMA async Tensor Cores and TMA, achieving [75-85% of H100 theoretical max FLOPS](https://tridao.me/blog/2024/flash3/) (740-850 TFLOPs) vs. 35% for FA2. Requires CUDA ≥12.3. Note that [logs don't distinguish FA2 vs FA3](https://github.com/vllm-project/vllm/issues/15344)—the version is determined silently based on GPU architecture.
