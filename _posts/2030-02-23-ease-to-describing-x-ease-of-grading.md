...
Competitive programming. This is easy to describe. Note that even if the task is very difficult for the average human, it does not mean that it is easy to describe.
...
However, if you give AI access to Minecraft and ask them to "build something great", or build "a church for themselves" the task is not easy to describe.
...
There are sites with information that contradict each other, there are information on the Internet that are unreliable or even intended to be false.
...
Maybe the change is just 50 lines of code, but there is a lot of context involved. This is not easy to describe.

Kaggle competitions. If you limit the information accessible to only the provided data, the task is easy to describe.
But you cannot do well in Kaggle competition if you only have access to the provided data.
Even though all Kaggle competitions have a problem statement, this doesn't mean that all the data you need to do well at a Kaggle competition is self-contained. In some competitions, you need to compile your own external data to gain an advantage. This depends on the competition. (explain?)
...
Competitive programming. This is easy to grade. (Explain)
...
However, if you give AI access to Minecraft and ask them to "build something great", or build "a church for themselves" the task is not easy to grade.
It is easier to grade if you ask AI to replicate a certain object in Minecraft.

Whether you do prefer one response over another. I think this is difficult to grade. (explain)
...
Scoring solutions to IMO problems based on a rubric.
...
I would expect the system to easily updated to solve the discovered task.
