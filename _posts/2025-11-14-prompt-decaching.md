...
This is anaglous to how humans forget parts of the conversation that they are in.
...
- Then Claude Code essentially resets the conversation with the summarize information
...
- The performance of a restarted conversation will have no chance at matching conversation that is allowed to continue.
    In a conversation, you would rather continue talking to the same person, rather than another person which only has a summary of the conversation.

What I propose here - is to delete parts of the conversation in-place.
...
- Irrelevant parts of the user query. If the user copy a very long piece of logs, probably just consider the relevant a part of the logs.
...
- Hook responses that are temporary (If the hooks asks to remind something but the reminder is not applicable, we can forget about the reminder)
...
Position IDs will continue to increment. Probably we should have rotational embeddings that is able to accomodate up to billions in length.
...
While there is some time involved in moving the memory, this should be neglible compared to the au

Implementation

- Paged attention - the KV blocks do not need to stored in a contiguous piece of memory.
...
- Anticipate that parts of the conversation will be deleted in the future.
    For example, at the end of the thought process, the model should summary the important points that should persist
...
You will annotate which parts of the prompt is NOT cached, and not cached since which turn.
...
If you want to interact talk to an AI live, you probably do not want the AI to stall at every 100k tokens.

With this we can continue investing in trasnformer architecture, and reuse all the tools used to optimize and interpret.
...
We need to each LLMs to accurately forget so that they can continuously accept tokens.
Getting LLMs to decache text would be the starter tasks.
