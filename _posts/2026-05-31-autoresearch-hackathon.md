...
I won third place.
The prize was $1,000 of Modal credits, and $1,000 of ChatGPT credits, and 6 months of ChatGPT Pro.
...
- Some neural network that was inspired from AlphaGo
- Ideal gameplay traces
- Submission pipeline

I made some preparations before the hackathon
- I made some "modified games". "Modified games" has the same engine logic with the games published from the Arc Foundation, but with slight modifications. For example, for the "original game" [ar25]((https://arcprize.org/tasks/ar25)), the shapes of the games will be different.
- I generated the ideal gameplay traces on the modified games.

#### Autoresearch objective

I only started the autoresearch system halfway during the hackathon. The purpose of the autoresearch system is to figure out a good neural network model that is able to predict actions.
...
I set up a system for the agent (Claude Code) to experiment with different model architecture that minimizes the validation loss.
...
They should should do some data analysis on the dataset and learn something.

Every model tested should be a standalone file that should not be modified after testing.
Initially I discovered that the agent modify the model architecture code after testing - this is bad because it will be very difficult to understand what exactly was the model architecture that produced a certain result.

Validation loss should only be calculated at the end.
I want to training to be done quickly so that the agent can iterate on model architecture.
Maybe I could set up a system to export the current model and calculate the validation loss at 25%/50%/75%/100% of the data without blocking the training process.

### This is some things I learnt
...
There was a model variant that hit 3.5 validation loss and for a while model variants cannot beat the score.
When the agent decided to rerun the training for the model variant, the validation loss is 3.9 instead.
...
Even if I am to scale the amount of data, I do not think the model can learn to play the quarter of the games it has trained on. (Note that for the ARC-AGI leaderboard, the model will need to solve games it has not seen.)
...
I also have to ensure that my ystem that works with multiple external abstractions - ARC SDK, Kaggle notebooks.
...
The agent modifies the architecture, runs the training script, and report the loss.
It does not understand why one model architecture is better than the other.
Optimizing merely for the validation loss may be optimal if you want to produce immeidate results, and this is the correct decision in a hackathon.
However, neither the agent or myself is learning anything, and we are very far from solving the main task for training a model to play games that it has seen in training.
