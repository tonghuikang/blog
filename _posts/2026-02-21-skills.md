...
For example, the commit message should contain these information
...
- What URLs needs to be included (for example Slack, Asana)
...
Instead of having the AI to inefficiently and likely inaccurately figure out the commit message standards in every session, the skill file documents the commit message standards, and the AI could skip the entire process of looking at many commit messages and failing unit test.
...
You have a resource that you want to provide access to your AI. The resource could be Notion, Slack, Asana, or any internal pages. Instead of playing telephone between the AI coding tool and the resource, you can write a skill that teaches AI how to read the resource. However, there is a prerequisite - this assumes that you AI coding tool has access to the resources, and you will have to get that set up first.
...
You want a process to be done more efficiently in the future. One of such process is oncall pages. You might already be handling oncall pages with AI tools that has access to dashes and error logs. In the future, you want to handle this more efficiently. You can write a skill that informs the model of the resources that they should look at and the dead ends that they should be aware of.
...
- Workflows that should have been a deterministic script. If I am writing a `check-commmit-message` skill, I should not be asking the model run checks where it could have been a unit tests. The LLM should not be an expensive linter. If there is still value in writing `check-commmit-message` to check the qualitative aspects on the commit message, the skill should ask the model to running the relevant unit test for the deterministic checks.
...
These are some ways I think is good evidence of testing

- If the skill output is a document, the resulting document could be an evidence.
...
- If the skill helps with an investigation, the investigation thread could be an evidence.
...
This means that you will have hundred of skill names, and hundred of skill descriptions. If each skill if 50 tokens, this will be 5000 tokens. Also depending on the quality of your skill descriptions, the model might invoke skills unnecessarily, or fail to invoke skills when it is needed.

If you look at the skills, there are skills are that company-wide and there are skills that there are team-wide. We should only load company-wide skills into context.

This can be done in Claude Code. For team-wide skills, I can add `disable-model-invocation: true` to prevent the skill for being loaded in context.
...
However, the skill standard requires all skill to be the same level.

So I have every skill folder to be a soft symlink to a skills/all.
...
`.claude/skills`, `.cursor/skills` and `.codex/skills` is a soft symlink to `skills/all`.


I hope this helps you to writing skills for you company, so that the agents could do things more accurately and efficiently.
