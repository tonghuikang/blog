...
If models want to write commit message accurately, without additional AI instructions they will need to figure out these requirements by looking at similar commits, or running the unit tests related to the commit message.
...
- Tasks that the AI could already solve accurately and efficiently. For example, you should not adding a skill on how to search the code, because the AI is likely already searching the code in the most efficient manner.[^search]
...
[^search]: That said, it might still be reasonable to incldue a skill that helps to search code if AI could not find the code.
     You might have some code that is hard to search, or example you might be searching for a string but the actual string is broken into two pieces with each string defined at difference placed.
     ...
     Of course, the better way to solve this is to avoid writing code that require doing this, or improving your codebase instead.


## Skill writing advice

I suggest start by writing the simplest possible skill that is worth using.
...
If you could, congratulations, you have found a good candidate to write a skill for.
...
Currently I have unit tests that check for that
- The skill name is short
- The skill description is follows convention [^description]
...
To help your colleagues write skills, you make it easy for them to write skills, with skills.
...
To help your colleagues improve skills, you make again make it easy for them to write skills, with skills.

There will be a `skill-feedback` skill where agents can provide feedback to skills.
