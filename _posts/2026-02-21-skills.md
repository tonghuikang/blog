...
It is the standardized format for AI instructions that I have been looking for.
...
When the model needs to write a commit message, it should decide to invoke a skill. When the skill is invoked, the model will read the skill content.
...
You have a resource that you want your AI to access.
...
You might already be handling on-call pages with AI tools that have access to dashboards and error logs.
In the future, you want to handle this more efficiently.
You can write a skill that informs the model of the resources that it should look at and the dead ends that it should be aware of.
...
- Workflows that should have been a deterministic script.
  If I am writing a `check-commit-message` skill, I should not be asking the model to run checks that could be unit tests.
